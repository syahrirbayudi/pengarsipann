...
class Administrasi extends CI_Controller
{
	...
	function data_puskesmas_view()
	{
		...
		if ($sesinya['level'] != '1') {
			...
			$data_update = mysql_query("select data_puskesmas.no_puskesmas,data_puskesmas.nama_puskesmas, ROUND(AVG(jumlah_fasilitas_puskesmas.jumlah_fasilitas)) AS jumlah_fasilitas_total,ROUND(AVG(jumlah_obat_puskesmas.jumlah_obat)) AS ketersediaan_obat_total,ROUND(AVG(jumlah_tujuan_loket.no_penyakit-20)) AS jumlah_pasien_total from data_puskesmasINNER JOIN jumlah_fasilitas_puskesmas on jumlah_fasilitas_puskesmas.no_puskesmas=data_puskesmas.no_puskesmas INNER JOIN jumlah_tujuan_loket on jumlah_tujuan_loket.no_puskesmas=data_puskesmas.no_puskesmas INNER JOIN jumlah_obat_puskesmas on jumlah_obat_puskesmas.no_puskesmas=data_puskesmas.no_puskesmas GROUP by data_puskesmas.no_puskesmas");
	...
	function data_puskesmas_edit($id = '')
	{
		...
		if ($sesinya['level'] != '1') {
			...
			$data = array(
				...
				'jumlah_pasien_total'			=> $data_puskesmas[0]['jumlah_pasien_total'],
				'ketersediaan_obat_total'			=> $data_puskesmas[0]['ketersediaan_obat_total'],
				'jumlah_fasilitas_total'			=> $data_puskesmas[0]['jumlah_fasilitas_total'],
	...
	function data_jumlah_tujuan_loket_view()
	{
		...
		if ($sesinya['level'] != '1') {
			...
			// $data_jumlah_tujuan_loket = $this->adminmodel->selectdata('jumlah_tujuan_loket order by no_jumlah_penyakit desc')->result_array();

			$data_jumlah_tujuan_loket = $this->adminmodel->selectdata('jumlah_tujuan_loket INNER JOIN data_tahun ON jumlah_tujuan_loket.no_tahun = data_tahun.no_tahun INNER JOIN data_loket on jumlah_tujuan_loket.no_loket = data_loket.no_loket INNER JOIN data_tujuan on jumlah_tujuan_loket.no_penyakit=data_penyakit.no_penyakit LEFT JOIN data_obat on data_penyakit.no=data_obat.no')->result_array();
			...
			$data = array(
				...
				'nama'			=> $sesinya['nama'],
				'data_jumlah_tujuan_lokett'		=> $data_jumlah_tujuan_loket,
				'titlesistem'	=> $this->model->getTitle(),
			);
	...
	function data_jumlah_tujuan_loket_save()
	{
		if ($_POST) {
			...
			if ($status == 'baru') {
				...
				$data = array(
					'no_jumlah_tujuan'	=> $no_jumlah_tujuan,
					'no_loket'	=> $no_loket,
					'nama_penumpang'	=> $nama_penumpang,
					'no_tujuan'	=> $no_tujuan,
					'no_tahun'	=> $no_tahun,
	...
	function data_jumlah_tujuan_loket_edit($id = '')
	{
		...
		if ($sesinya['level'] != '1') {
			...
			$data = array(
				...
				'status'			=> 'edit',
				'no_tahun'			=> $pilih_tahun,
				'no_puskesmas'			=> $pilih_loket,
				'nama_pasien'				=> $jumlah_tujuan_loket[0]['nama_penumpang'],
				'no_penyakit'			=> $pilih_tujuan,
